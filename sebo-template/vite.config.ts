...
function marcaDoCliente(): Plugin {
  ...
  const arquivos: Record<string, { tipo: string; texto: string }> = {
    ...
    'favicon.svg': {
      tipo: 'image/svg+xml',
      texto: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="8" fill="${seo.faviconBg}"/><path d="M14 10h26l10 10v34H14z" fill="${seo.faviconFg}"/><path d="M40 10v10h10z" fill="${seo.faviconBg}" opacity=".35"/><text x="32" y="46" text-anchor="middle" font-family="Georgia, serif" font-size="26" font-weight="700" fill="${seo.faviconBg}">${esc(site.initial)}</text></svg>
`,
    },
  }
  return {
