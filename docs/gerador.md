...
Preservadas como demonstração (restaurante: `public/demo/`; clínica: URLs do Pexels em `images.ts`). Nada é baixado nem criado. `public/demo/og.jpg` do restaurante não é copiado e `seo.ogImage` fica vazio.

## Limitações (manual depois de gerar)

- Todo `src/data/conteudo.ts` é texto de demonstração (no restaurante cita "Cumaru"; o gerador avisa quais arquivos contêm o nome do template).
