...
export default function Faq() {
  ...
  return (
    <section id="perguntas" className="py-20 lg:py-28">
        ...
        <Reveal delay={100} className="lg:col-span-7">
          <ul className="border-t border-border">
            {faq.itens.map((f, i) => {
              ...
              return (
                <li key={f.pergunta} className="border-b border-border">
                  <h3>
                    <button
                      type="button"
                      id={`faq-botao-${i}`}
                    ...
                    >
                      {f.pergunta}
                      <IconPlus className={`size-6 shrink-0 text-accent transition-transform duration-300 ${ativa ? 'rotate-45' : ''}`} />
                    </button>
                  </h3>
                  <div
                    id={`faq-${i}`}
