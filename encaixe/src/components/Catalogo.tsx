...
export default function Catalogo({ selecionado }: Props) {
  const tipo = encaixes.find((e) => e.id === selecionado)
  const filtradas = tipo ? pecas.filter((p) => tipo.match(p.encaixe)) : pecas

  return (
    <section id="catalogo" className="scroll-mt-24 border-b border-line px-6 py-20 md:py-28">
      <div className="mx-auto max-w-5xl">
        <h2 className="font-heading text-3xl font-medium text-ink sm:text-4xl">
          Peças em {tipo?.nome.toLowerCase() ?? 'todos os encaixes'}
        </h2>
        <p className="mt-3 max-w-md text-ink/70">
          {filtradas.length} de {pecas.length} peças de referência usam esse encaixe — ponto de partida, não tabela fechada.
        </p>

        {filtradas.length === 0 ? (
          <p className="mt-10 text-ink/60">Nenhuma peça de referência usa esse encaixe ainda — pergunte, a gente desenha.</p>
        ) : (
