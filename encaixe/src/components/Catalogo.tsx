  ...
  return (
    <section id="catalogo" className="scroll-mt-24 border-b border-line px-6 py-20 md:py-28">
      <div className="mx-auto max-w-5xl">
        <h2 className="font-heading text-3xl font-medium text-ink sm:text-4xl">Peças de referência</h2>
        <p className="mt-3 max-w-md text-ink/70">
          Ponto de partida pra conversa, não tabela fechada — tecido e corte
          mudam pra cada encomenda (use o configurador acima pra outras combinações).
        </p>
          ...
          {pecas.map((p) => (
            <article key={p.nome} className="group">
              <div className="aspect-[10/7] bg-paper-deep/40 p-4 transition-transform duration-300 group-hover:-translate-y-1">
                <DesenhoTecnico desenho={p.desenho} nomeJunta={p.detalhe} />
              </div>
              <h3 className="mt-5 font-heading text-xl font-medium text-ink">{p.nome}</h3>
              <p className="text-sm text-ink/55">
                {p.categoria} em {p.tecido}
              </p>
              ...
              <button
                type="button"
                onClick={() => sendToWhatsApp(`Olá! Quero um orçamento pra ${p.nome} (${p.tecido}, ${p.medidas.toLowerCase()}).`)}
