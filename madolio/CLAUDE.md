...
**Ajuste seguinte:** o usuário reportou não conseguir "acessar" o site — achava que estava saindo dele ao usar `/projetos`. A grade antiga (`ProjectsGrid.tsx`) envolvia o card inteiro num `<a href={url} target="_blank">`: tocar em qualquer lugar do card abria o site externo na hora, sem aviso. Ele pediu que os previews (a lista mestre-detalhe com `LivePreview`) aparecessem também em `/projetos`. Resolvido extraindo a lista mestre-detalhe (que só existia em `Trabalhos.tsx`) pra `ProjectShowcase.tsx` — um componente sem `<section>`/heading próprios, só a lista + o painel — usado agora tanto em `Trabalhos.tsx` (home) quanto em `pages/Projetos.tsx`. `ProjectsGrid.tsx` e `SiteMock.tsx` (o mockup abstrato de navegador) ficaram sem uso e foram removidos — todo lugar que mostra projeto usa `LivePreview` (iframe real) agora, e só o link "Abrir site completo" navega pra fora.
