...
function marcaDoCliente(): Plugin {
  ...
  const arquivos: Record<string, { tipo: string; texto: string }> = {
    ...
    'favicon.svg': {
      tipo: 'image/svg+xml',
      texto: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="32" fill="${seo.faviconBg}"/><text x="32" y="43" text-anchor="middle" font-family="Georgia, serif" font-size="34" font-style="italic" fill="${seo.faviconFg}">${esc(site.initial)}</text></svg>\n`,
    },
  }
  return {
