...
import { reserva, rotulos } from '../data/conteudo'
    ...
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-3 px-5 lg:h-20 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <a href="#reservas" className="btn btn-primary !min-h-11 !px-4 sm:!px-5" onClick={() => setAberto(false)}>
            {rotulos.reservar}
          </a>
          <button
            ref={botaoMenu}
            type="button"
            className="grid size-11 place-items-center rounded-full border border-foreground/25 lg:hidden"
      ...
      <nav id="menu-mobile" aria-label="Menu móvel" hidden={!aberto} className="border-t border-border bg-background lg:hidden">
        <ul className="mx-auto max-w-6xl px-5 py-3">
          ...
          <li className="py-4">
            <a href={whatsappUrl(reserva.mensagem)} target="_blank" rel="noreferrer" className="btn btn-primary w-full">
              {reserva.botao}
            </a>
          </li>
        </ul>
