...
export function Logo({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <span className="flex min-w-0 items-center gap-3">
      <span
        aria-hidden="true"
        className="grid size-10 shrink-0 place-items-center rounded-full border border-accent max-[359px]:hidden font-display text-xl text-accent italic"
      >
        {site.initial}
      </span>
      <span className="flex min-w-0 flex-col leading-none">
        <span className="truncate font-display text-2xl tracking-tight">{site.name}</span>
        <span
          className={`mt-1 text-[0.6875rem] font-semibold tracking-[0.18em] text-muted uppercase ${compact ? 'hidden min-[480px]:block' : ''}`}
        >
          {site.descriptor}
        </span>
      </span>
