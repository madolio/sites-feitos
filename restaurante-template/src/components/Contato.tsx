          ...
          <Reveal className="min-w-0 lg:col-span-6">
            <ul className="divide-y divide-border border-y border-border">
              {itens.map((i) => (
                <li key={i.rotulo}>
                  <a href={i.href} className="group flex min-h-20 items-center gap-4 py-4">
                    <span className="grid size-11 shrink-0 place-items-center rounded-full bg-background text-accent transition-colors [@media(hover:hover)]:group-hover:bg-accent [@media(hover:hover)]:group-hover:text-accent-foreground">
                      {i.icone}
                    </span>
              ...
              <li className="flex items-start gap-4 py-4">
                <span className="grid size-11 shrink-0 place-items-center rounded-full bg-background text-accent">
                  <IconClock />
                </span>
                <span className="min-w-0">
          ...
          <Reveal delay={100} className="min-w-0 lg:col-span-6">
            {site.mapEmbedUrl ? (
              <iframe
                ...
                className="h-full min-h-[25rem] w-full rounded-3xl border-0"
              />
            ) : (
              /* Mapa estilizado (sem iframe, sem dado real): defina mapEmbedUrl em site.ts para usar o mapa real */
              <div className="relative flex h-full min-h-[25rem] flex-col justify-end overflow-hidden rounded-3xl border border-border bg-background p-6 sm:p-8">
                ...
                <span aria-hidden="true" className="absolute top-[24%] left-1/2 grid size-14 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-accent text-accent-foreground">
